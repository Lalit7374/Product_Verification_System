...
const QuerySeller = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrorMsg('Failed to fetch product details. Please try again.');
  ...
  return (
    <div>
      ...
      <nav className="bg-gray-800 text-white">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <a href="/" className="text-xl font-bold">
            Product Verification System <span className="text-yellow-400">using Secure Digital Ledger Technology</span>
          </a>
          <ul className="flex space-x-6">
            <li><a className="hover:text-yellow-400" href="/">Home</a></li>
            <li><a className="hover:text-yellow-400" href="/manufacturer">Manufacturer</a></li>
      ...
      <section className="max-w-4xl mx-auto mt-10 px-4">
        ...
        <button
          onClick={handleSubmit}
          className={`bg-yellow-500 hover:bg-yellow-600 text-white px-6 py-2 rounded transition ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          disabled={loading}
        >
      ...
      {/* Error Message */}
      {errorMsg && (
        <section className="max-w-4xl mx-auto mt-4 px-4">
